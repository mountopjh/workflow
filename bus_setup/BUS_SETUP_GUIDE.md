# 大总管（Workbuddy）总线搭建指南

> 大总管（Workbuddy）在本工作流中的角色是**纯信号总线**，绝不执行业务代码。
...
### 4.4 Shadow/ 残留信号
启动 大总管（Workbuddy） 主循环前，建议先**清空** `Shadow/` 目录里的旧 `TRIGGER_*.md`（用 `bootstrap_reset.bat`）。否则可能上来就跑错路由。
...
3. 启动 大总管（Workbuddy） 主循环（按本指南配置好的工作流）
